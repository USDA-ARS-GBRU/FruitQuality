...
wandb_logger = WandbLogger(project="Riverside-runs", offline=False, log_model=True)
MODEL_BASE = "nvidia/segformer-b3-finetuned-ade-512-512"
EPOCHS = 200
WEIGHT = "None"
MASK_TYPE = "None"
...
#                           for image_id in self.ids]
        self.images_fps = [os.path.join(
            "RiversideFrtInterior", f"{id_}") for id_ in ids]
...
mtl_segformer_finetuner.model.save_pretrained("tmp/model")
artifact = wandb.Artifact(name="my_model", type="model")
# Add dataset directory to artifact
artifact.add_dir(local_path="tmp/model")
wandb.log_model(artifact) 

mtl_segformer_finetuner = mtl_segformer_finetuner.to(device="cuda").eval()

def run_sample_output(sample_dataset: SemanticSegmentationDataset, img_id):
    idx_in_test_set = sample_dataset.get_idx_of_img_id(img_id)
    if idx_in_test_set == -1:
        return None
    dataset_image = sample_dataset.get_image(idx_in_test_set)
    encoded_inputs = sample_dataset[idx_in_test_set]
    ...
    ground_truth_mask = (np.array(sample_dataset.get_mask(idx_in_test_set)) // 51)
...
#     seed_data = run_sample_output(test_dataset, img_id)
#     if seed_data:
...
# River Output
def run_riverside_output(river_dataset: RiversideDataset, img_id):
    idx_in_test_set = river_dataset.get_idx_of_img_id(img_id)
    if idx_in_test_set == -1:
        return None
    dataset_image = river_dataset.get_image(idx_in_test_set)
    encoded_inputs = river_dataset[idx_in_test_set]
    images = encoded_inputs['pixel_values']
    # outputs = segformer_finetuner.model(images.unsqueeze(0), masks.unsqueeze(0))
    images = images.to(device="cuda")
    seg_outputs, clf_outputs = mtl_segformer_finetuner.model(
        images.unsqueeze(0), None, None)

    logits = seg_outputs[0]
    clf_logits = clf_outputs[0]
...
river_dataset = RiversideDataset(
    river_ids,None, None, CLASSES, feature_extractor
)
columns = ["image id", "seed prediction"]
seed_count_data = []
for img_id in river_ids:
    seed_data = run_riverside_output(river_dataset, img_id)
    if seed_data:
        seed_count_data.append(seed_data)
